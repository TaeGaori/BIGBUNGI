...
# 결측치 처리, 이상치 확인, 인코딩, 스케일링 , 머신러닝 모델 선정
...
# 결측치가 잇는 컬럼(열) 자체를 삭제 --> axis=1 --> 열 기준
...
# (최빈값 대체와 달리, "결측이었따는 사실 자체"도 모델이 학습할 수 있게 한다.)
...
# 주당근무시간은 이상치에 덜 민감한 중앙값(madian)으로 채우기
...
# print(test[test['age'] <= 0])   #확인만 한다. 이상치처리하지 않는다(삭제하지 않는다.)
...
# 인코딩 (범주형 -> 숫자)
# 원-핫 인코딩
# 판다스의 get_dummies(): 범주형 컬럼들을 0/1 로 이루어진 여러 컬럼으로 자동 변환
...
# 주의 !! train과 test를 따로 원핫인코딩하면 카테고리 값의 종류가 서로 달라 생성되는 더미 컬럼의 개수/이름이 어긋날 수 있다.
#   (예: train에만 있는 특정 국가명)  이를 해결하기 위해 train+test 합친 뒤 한 번에 인코딩 한다.
#   다시 분리하는 것이 안전한 방법
...
# 여러 스케일러를 비교 실습을 할 때, 이전 스케일링이 누적되지 안도록 원본을 복사해서 시작
...
# fosejavhfptmxm /LightGBM 같은 트리 기반 모델은 스케일링이 성능에 큰 영향을 주지 않는
...
# print(accuracy) # 0.868793721207985

# f1_score : 정밀도와 재현율의 조화평균 --> 클래스 불균형 데이터 정확도보다 신회할 수 있다.
f1 = f1_score(y_val, pred, pos_label='>50K')
# print(f1)   # 0.6910405785456006


#---------------------------------------------------------------------------
# LightGBM : 그라디언트 부스팅 계열, 오답노트, 정형 데이터엣허 비교적 랜덤포레스트보다 성능이 뛰어나다.

import lightgbm as lgb

# verbose=-1 --> 학습 중 불필요한 로그 메시지는 출력은 인한다.
lgbmc = lgb.LGBMClassifier(random_state=0, verbose=-1)
lgbmc.fit(X_train, y_train)
pred1 = lgbmc.predict_proba(X_val)  #확률
roc_auc = roc_auc_score(y_val, pred1[:,1])
print(roc_auc)  # 0.9278203048419168

pred2 = lgbmc.predict(X_val)
accuracy = accuracy_score(y_val, pred2)
f1 = f1_score(y_val, pred2, pos_label='>50K')
print(accuracy) # 0.8752772564408804
print(f1) # 0.7111813512445674